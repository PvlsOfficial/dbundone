...
const getWorkerPool = () => {
  ...
  try {
    const { createDefaultWorkerPool } = require('./workerPool')
    workerPool = createDefaultWorkerPool()
  } catch (err) {
    console.error('Failed to create worker pool:', err)
  }
  return workerPool
}
...
async function scanForFLPFiles(folderPath: string, maxDepth: number = 10): Promise<string[]> {
  ...
  while (scanQueue.length > 0) {
    ...
    // Send progress update every 10 directories scanned
    if (dirsScanned % 10 === 0) {
      mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
        current: 0,
        total: 1,
        daw: "FL Studio",
        file: `Scanning directories... (${dirsScanned} scanned)`,
        isScanning: true,
        phase: "scanning"
      })
    }
  ...
  console.log(`Found ${flpFiles.length} FLP files in ${dirsScanned} directories`)
  return flpFiles
}
...
async function extractFLPMetadataBatchOptimized(flpPaths: string[]): Promise<Record<string, any>> {
  ...
  for (let i = 0; i < flpPaths.length; i += CHUNK_SIZE) {
    const chunk = flpPaths.slice(i, i + CHUNK_SIZE)
    const chunkNumber = Math.floor(i / CHUNK_SIZE) + 1
    const totalChunks = Math.ceil(flpPaths.length / CHUNK_SIZE)
    
    console.log(`[Optimized Batch] Processing chunk ${chunkNumber}/${totalChunks} (${chunk.length} files)`)
    
    // Update progress to show chunk processing
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      current: i,
      total: flpPaths.length,
      daw: "FL Studio",
      file: `⚡ Processing chunk ${chunkNumber}/${totalChunks} (${chunk.length} files)...`,
      isScanning: true,
      phase: "extracting_metadata"
    })
    
    const chunkMetadata = await extractFLPMetadataChunk(chunk, i, flpPaths.length)
    Object.assign(metadataMap, chunkMetadata)
  }
  
  console.log(`[Optimized Batch] Successfully extracted metadata for ${Object.keys(metadataMap).length} files total`)
  return metadataMap
}

async function extractFLPMetadataChunk(flpPaths: string[], chunkOffset: number, totalFiles: number): Promise<Record<string, any>> {
  return new Promise((resolve, reject) => {
    ...
    currentPythonProcess = spawn(pythonExecutable, [scriptPath, '--batch', ...flpPaths], {
    ...
    if (currentPythonProcess.stderr) {
      currentPythonProcess.stderr.on('data', (data) => {
        ...
        // Try to parse progress updates from stderr
        try {
          ...
          for (const line of lines) {
            if (line.trim()) {
              ...
              if (progressData.type === 'progress') {
                // Adjust progress to account for chunk offset
                const overallCurrent = chunkOffset + progressData.current
                mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
                  current: overallCurrent,
                  total: totalFiles,
                  daw: "FL Studio",
                  file: `⚡ Extracting metadata... ${overallCurrent}/${totalFiles} files processed`,
                  isScanning: true,
                  phase: "extracting_metadata"
                })
              }
          ...
          // Not a progress update, just accumulate stderr
        }
      })
    }

    const sendFilePaths = async () => {
      // File paths are now passed as command line arguments, no need to send via stdin
    }
      ...
      if (code === 0) {
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            reject(new Error(`Unexpected Python output format: expected array, got ${typeof parsed}`))
          }
        } catch (e: any) {
...
async function scanFLFolderInternal(folderPath: string) {
  try {
    ...
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      ...
      total: 1,
      daw: "FL Studio",
      file: "🔍 Scanning folders for FLP files...",
      isScanning: true,
      phase: "scanning"
    })
    ...
    // Phase 2: Filtering autosaves
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      current: 0,
      total: flpFiles.length,
      daw: "FL Studio",
      file: `📁 Found ${flpFiles.length} FLP files, filtering autosaves...`,
      isScanning: true,
      phase: "filtering"
    })
    ...
    if (filteredFiles.length === 0) {
      mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
        ...
        total: 0,
        daw: "FL Studio",
        file: "✅ No new FLP files to process",
        isScanning: false,
        phase: "complete"
      })
    ...
    // Phase 3: Extracting metadata in parallel (FAST)
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      current: 0,
      total: filteredFiles.length,
      daw: "FL Studio",
      file: `⚡ Extracting metadata from ${filteredFiles.length} FLP files in parallel...`,
      isScanning: true,
      phase: "extracting_metadata"
    })
    ...
    try {
      ...
      mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
        ...
        total: filteredFiles.length,
        daw: "FL Studio",
        file: "⚠️ Metadata extraction failed, proceeding with basic info...",
        isScanning: true,
        phase: "metadata_failed"
      })
    }
    
    // Phase 4: Checking existing projects
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      current: 0,
      total: filteredFiles.length,
      daw: "FL Studio",
      file: `🔍 Checking ${filteredFiles.length} files against existing projects...`,
      isScanning: true,
    ...
    // Phase 5: Processing and saving to database
    mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
      current: 0,
      total: filteredFiles.length,
      daw: "FL Studio",
      file: `💾 Saving ${filteredFiles.length} projects to database...`,
      isScanning: true,
    ...
    await processBatch(filteredFiles, 50, async (batch) => {
      for (const flpPath of batch) {
        try {
          ...
          // Send progress update every 10 files for more frequent updates
          if (processedCount % 10 === 0 || processedCount === filteredFiles.length - 1) {
            const action = existingProject ? "updating" : "adding"
            mainWindow?.webContents?.send(IPC_CHANNELS.SCAN_PROGRESS, {
              current: processedCount + 1,
              total: filteredFiles.length,
              daw: "FL Studio",
              file: `💾 ${action === "updating" ? "Updating" : "Adding"} project ${processedCount + 1}/${filteredFiles.length}: ${filename}`,
              isScanning: true,
              phase: "saving_projects"
            })
          }
          
          const metadata = metadataMap[flpPath] || null
          const projectName = metadata?.title || path.basename(flpPath, ".flp")
    ...
    // Send final progress update to indicate scanning is complete
    const completionMessage = addedCount > 0 && updatedCount > 0 
      ? `✅ Scan complete! Added ${addedCount} new projects, updated ${updatedCount} existing projects`
      : addedCount > 0 
        ? `✅ Scan complete! Added ${addedCount} new projects`
        : updatedCount > 0
          ? `✅ Scan complete! Updated ${updatedCount} existing projects`
...
async function updateFileModDatesInternal() {
  try {
    ...
    await processBatch(projects, 100, async (batch) => {
...
async function rescanFLPMetadataInternal() {
  ...
  await processBatch(flpProjects, 100, async (batch) => {
    const updatePromises = batch.map(async (project) => {
      try {
        const metadata = metadataMap[project.dawProjectPath!] || null
        const updates: any = {}
...
function setupIpcHandlers(): void {
  ...
  // Generic worker runner - run CPU-bound tasks in the worker pool
  ipcMain.handle('worker:run', async (_event, task: string, data: any) => {
    const pool = getWorkerPool()
    if (!pool) return { error: 'no_worker_pool' }
    try {
      const res = await pool.runTask(task, data)
      return { result: res }
    } catch (err: any) {
